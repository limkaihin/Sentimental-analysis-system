...
with tab1:
    st.subheader("Input")
    # --- Input source: Manual or Upload files (.txt/.csv) ---
    ...
    # --- Optional: split glued words (iamsad -> i am sad) using dictionary segmentation ---
    seg_enabled = st.checkbox("Split glued words (e.g., iamsad → i am sad)", value=True)
    # Build vocab: AFINN words + a small set of common words
    common = {"i","am","is","are","was","were","be","being","been","very","not","no","good","bad","sad","happy","movie","acting","the","a","an"}
    vocab = set(afinn.keys()) | common
    set_word_segmentation(seg_enabled, vocab)
