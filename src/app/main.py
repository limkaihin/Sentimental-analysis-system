...
def parse_args() -> argparse.Namespace:
    ...
    - data/reviews contains sample review .txt/.gz files.
    ...
    p.add_argument("--reviews_subdir", type=str, default="reviews", help="Subfolder under data_dir with .txt or .gz reviews")
    ...
    p.add_argument("--window_size", type=int, default=3, help="Sliding window size k (tokens or sentences)")
    p.add_argument("--unit", type=str, default="token", choices=["token", "sentence"], help="Window unit: token or sentence")
...
def main() -> None:
    ...
    # Load data and lexicons (.txt and .gz supported by read_text_files)
    ...
    k = args.window_size
    per_review_windows: List[List[tuple[int, int]]] = sliding_window_sentiment_analysis(
        reviews, k, afinn, emoticons, unit=args.unit
    )
